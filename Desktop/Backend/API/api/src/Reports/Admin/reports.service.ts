...
export class ReportsService {
  ...
  // 📌 5) Aging buckets analytics
  //   ✔ bucket 0–7 days = فواتير جديدة
  // ✔ bucket 8–30 days = لازم تتراجع/تدفع
  // ✔ bucket 31–60 = overdue
  // ✔ bucket 60+ = risk / red zone 🚨
async getAgingBuckets() {
  ...
  for (const inv of invoices) {
    ...
    // ===================================
    // CASE 1️⃣ — ReadyForPaid (بدون أقساط)
    // ===================================
    ...
    // ===================================
    // CASE 2️⃣ — Partial_paid (مع أقساط)
    // ===================================
    if (inv.status === "Partial_paid") {
      let po: PurchaseOrder | null= null;

      // نتأكد من أن po_number ليس null
      if (inv.po_number) {
      ...
      // لو ما في أقساط → عوملها زي ReadyForPaid
      ...
      // ⚠️ لو diffDays سلبي → مش متأخرة → تجاهلها
      if (diffDays < 0) continue;
    }

    // ===================================
    // وضع الفاتورة في البكيت الصحيح
    // ===================================
    if (diffDays !== null) {
...
async getPaymentDelay(year: number) {
  // 1️⃣ نجلب فقط دفعات السنة المطلوبة + المكتملة
  const payments = await this.paymentModel.findAll({
  ...
  // 2️⃣ لكل دفعة → نجيب الفاتورة بشرط السنة
  for (const payment of payments as any[]) {
    ...
    // ⚠️ نجيب الفاتورة لكن بشرط سنة الفاتورة
    ...
    // 3️⃣ نبحث إذا يوجد تقسيط لـ PO لكن بشرط سنة PO أيضًا
    ...
    // ⭐ بدون تقسيط → التأخير = يوم الدفع - يوم الفاتورة
    const delay = payDate.diff(invoiceDate, "day");
    if (delay > 0) {
      delays.push(delay);
      monthlyMap[payDate.month()].push(delay);
...
async getMonthlyPayments(year: number) {
    ...
    // Helper: monthly payments لسنة واحدة
  ...
  const items = await this.invoiceItemModel.findAll({
    attributes: [
      [fn('LOWER', col('SupplierInvoiceItem.item_name')), 'normalized_name'], // 👈 توحيد الاسم
    ...
    include: [
      {
        ...
        where: {
          status: {
            [Op.in]: ['ReadyForPaid', 'Paid', 'Partial_paid'], // 👈 فقط المعتمدة
          },
  ...
  return items.map((i: any) => ({
    name: i.normalized_name,       // الإسم الموحد للمنتج
...
async getPaymentMethodStats(year) {
  // نجيب الفواتير فقط اللي لها طريقة دفع
  const invoices = await this.invoiceModel.findAll({
    ...
    where: {
      ...
      // [Op.and]: where(fn("YEAR", col("payment_date")), year)
    },
    raw: true
  });

  // نرجّع بشكل صافي للفرونت حسب ما بدو
  return invoices.map(inv => ({
    paymentMethod: inv.payment_method,
    status:
      inv.status === "Paid"
        ? "Paid"
        : "Unpaid" // ReadyForPaid + Partial_paid
  }));
}

/**
 * جمع البيانات الإحصائية من قاعدة البيانات لتحليلها بواسطة AI
 */
...
/**
 * توليد AI Insights بناءً على البيانات الفعلية
 */
async getAIInsights(): Promise<AIInsight[]> {
  try {
    ...
    // إذا كان عدد الـ insights قليلاً، أضف defaults
    if (insights.length < 4) {
      const defaults = this.getDefaultInsights(analyticsData);
      insights = [...insights, ...defaults.slice(insights.length)];
    }

    return insights.slice(0, 8); // حد أقصى 8 insights
  } catch (error) {
    console.error('Error generating AI insights:', error);
    // في حالة الخطأ، نرجع insights افتراضية
    const analyticsData = await this.collectAnalyticsData();
...
/**
 * توليد insights افتراضية بناءً على البيانات
 */
private getDefaultInsights(analyticsData: any): AIInsight[] {
  const insights: AIInsight[] = [];
  let id = 1;

  // 1. Revenue growth prediction
  ...
  // 4. Purchase timing optimization recommendation
  ...
  // 6. Cost reduction prediction (if there's improved performance)
...
/**
 * حساب نسبة النمو من بيانات شهرية
 */
